...
import { changeFinder, changeFinderInline } from "../utils/changeFinder";
import { IncidentInputValidator } from "../validators/zodValidators";
const prisma = new PrismaClient();
export const controller = {
  fetchincident: async (req: Request, res: Response, next: NextFunction) => {
  ...
  updateIncident: async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const result = await prisma.incidentUpdate.create({
        data: {
          incidentId: Number(req.params.id),
          userId: Number(req.body.by),
          message: req.body.message,
          updateType: req.body.incidentType,
        },
      });
    ...
    try {
      if (req.body.from === "MAIN_UPDATE") {
        ...
        const {
          ...
          incidentUpdateLoge: Record<IncedentUpdateField, any>[];
          changed: Partial<Incident>;
          updatedFiedlsLogs: Record<LogField, any>[];
        } = changeFinder(data, req.body);
        const updatedData = reportModification(
          changed,
          id,
          incidentUpdateLoge,
          updatedFiedlsLogs
        ...
        const { id } = req.params;
        const orginalValue = await prisma.incident.findUnique({
          where: { id: Number(id) },
          include: { assignedTo: true },
        });
        const isChanged = changeFinderInline(orginalValue, req.body);
        if (!isChanged) {
          return;
        }
        if (orginalValue) {
          const result = await inLineUpdate(
            { ...req.body, id, user: "2" },
            orginalValue
          );
          res.json(result);
